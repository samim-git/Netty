...
public class ServerHandler<T extends NettyMessage> extends ChannelInboundHandlerAdapter {

    private final Logger logger = getLogger(ServerHandler.class);
    private final List<NettyMessageListener<T>> messageListeners = new CopyOnWriteArrayList<>();
    private final boolean failOnError;

    public ServerHandler(boolean failOnError) {
    ...
    @SafeVarargs
    public ServerHandler(NettyMessageListener<T>... listeners) {
        this(true);
...
//                    logger.debug("Handling IsoMessage[@type=0x{}] with {}",
//                            String.format("%04X", nettyMessage.getType()), messageListener);
